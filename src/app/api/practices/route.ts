...
export async function POST(request: Request) {
  try {
    ...
    const { type, category, count = 10, libraryName, title: customTitle } = data;
    ...
    switch (type) {
      ...
      case 'custom':
        // 自定义题库：使用指定题库名称的题目
        if (!libraryName) {
          return NextResponse.json({ error: 'Library name is required for custom practice' }, { status: 400 });
        }
        ...
        // 如果没有找到题目，可能是因为标签格式问题，尝试另一种查询方式
        if (!questions || questions.length === 0) {
          console.log(`未找到标签为 "${libraryName}" 的题目，尝试其他查询方式`);
          questions = await Question.find({
            $and: [
              { tags: '自定义' },
              { tags: libraryName }
        ...
        // 如果还是没有找到题目，则返回错误
        if (!questions || questions.length === 0) {
          console.error(`未找到题库 "${libraryName}" 中的题目`);
          return NextResponse.json({ 
            error: `未找到题库 "${libraryName}" 中的题目`,
            libraryName
          }, { status: 404 });
        }
        
        console.log(`找到题库 "${libraryName}" 中的题目数量:`, questions.length);
