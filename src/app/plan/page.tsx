...
function PlanWithSearchParams({ onPlanIdReceived }: { onPlanIdReceived: (planId: string | null) => void }) {
  const searchParams = useSearchParams();
  const planId = searchParams.get('id');
  
  // 通知父组件planId
  onPlanIdReceived(planId);
...
export default function PlanPage() {
  ...
  useEffect(() => {
    ...
    const loadPlan = async () => {
      ...
      } else if (planId === null && status === 'authenticated') {
        // 如果没有planId，重定向到计划列表
        router.push('/plans')
      }
    }
    ...
    loadPlan()
  }, [status, session, router, fetchPlans, planId])
    ...
    return (
      <div className="flex justify-center items-center min-h-screen">
        <Suspense fallback={null}>
          <PlanWithSearchParams onPlanIdReceived={setPlanId} />
        </Suspense>
